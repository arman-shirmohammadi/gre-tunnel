...
change_gre_ip() {
  local id local_ip remote_ip new_ip which side

  mapfile -t GRE_IDS < <(get_gre_ids)
  local -a GRE_LABELS=()
  local id_tmp
  for id_tmp in "${GRE_IDS[@]}"; do GRE_LABELS+=("GRE${id_tmp}"); done

  if ! menu_select_index "Change GRE IP" "Select GRE tunnel:" "${GRE_LABELS[@]}"; then
    return 0
  fi
  id="${GRE_IDS[$MENU_SELECTED]}"
  ...
  local unit="/etc/systemd/system/gre${id}.service"
  [[ -f "$unit" ]] || { die_soft "Unit file not found: $unit"; return 0; }

  local current_line
  current_line=$(grep -E '^ExecStart=/sbin/ip tunnel add gre'"${id}"' mode gre local' "$unit" | head -n1)

  if [[ -z "$current_line" ]]; then
    die_soft "Cannot find tunnel add line in unit file."
    return 0
  fi

  local cur_local cur_remote
  cur_local=$(echo "$current_line" | grep -oP '(?<=local )\S+')
  cur_remote=$(echo "$current_line" | grep -oP '(?<=remote )\S+')

  render
  echo "Current settings for GRE${id}:"
  echo "  Local  IP : ${cur_local}"
  echo "  Remote IP : ${cur_remote}"
  echo

  while true; do
    read -r -p "Change which one? (local / remote / both / cancel): " which
    which="$(trim "${which,,}")"
    case "$which" in
      local)   side="local";  break ;;
      remote)  side="remote"; break ;;
      both)    side="both";   break ;;
      cancel)  add_log "Change IP cancelled."; return 0 ;;
      *) add_log "Please enter: local, remote, both or cancel" ;;
    esac
  done

  render
  if [[ "$side" == "local" || "$side" == "both" ]]; then
    ask_until_valid "New LOCAL IP:" valid_ipv4 new_ip
    local_ip="$new_ip"
  else
    local_ip="$cur_local"
  fi

  if [[ "$side" == "remote" || "$side" == "both" ]]; then
    ask_until_valid "New REMOTE IP:" valid_ipv4 new_ip
    remote_ip="$new_ip"
  else
    remote_ip="$cur_remote"
  fi

  local new_tunnel_line
  new_tunnel_line="ExecStart=/sbin/ip tunnel add gre${id} mode gre local ${local_ip} remote ${remote_ip} key ${id}00 nopmtudisc"

  cp -a "$unit" "${unit}.bak-$(date +%s)" 2>/dev/null || true

  sed -i -E "s|^ExecStart=/sbin/ip tunnel add gre${id} mode gre local .*|${new_tunnel_line}|" "$unit"

  add_log "Updated tunnel endpoints in unit file:"
  add_log "  Old local : ${cur_local}  →  ${local_ip}"
  add_log "  Old remote: ${cur_remote} →  ${remote_ip}"

  if ip link show "gre${id}" &>/dev/null; then
    add_log "Applying live changes (best effort)..."
    systemctl stop "gre${id}.service"  >/dev/null 2>&1 || true
    ip link del "gre${id}" 2>/dev/null || true
    systemd_reload
    systemctl start "gre${id}.service" >/dev/null 2>&1 || add_log "WARNING: start failed after IP change"
  fi
  ...
  echo
  echo "Unit file updated → please check status"
  show_unit_status_brief "gre${id}.service"
  pause_enter
}
...
main_menu() {
  ...
  while true; do
    ...
    case "$choice" in
      ...
      9) add_log "Selected: Change IP"; change_gre_ip ;;
